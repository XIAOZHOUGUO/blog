---
title: setup：组件渲染前的初始化过程是怎样的？
category:
  - Vue3
tags:
  - setup
date: 2022-06-29 10:23:30
---
...
![](setup/Ciqc1F8VZpKAVYWOAABLt08AfuQ883.png)
...
我们要关注 createComponentInstance 方法的实现：
...
可以看到，我们从组件 vnode 中获取了 props、children、shapeFlag 等属性，然后分别**对 props 和插槽进行初始化**，这两部分逻辑在后续的章节再详细分析。根据 shapeFlag 的值，我们可以判断这是不是一个有状态组件，如果是则要进一步去设置有状态组件的实例。
...
function setupStatefulComponent (instance, isSSR) {
  ...
  if (setup) {
    ...
    const setupResult = callWithErrorHandling(setup, instance,
          0 /* SETUP_FUNCTION */, [instance.props, setupContext])
    // 处理 setup 执行结果
...
可以看到，HelloWorld 子组件的 setup 函数接收两个参数，第一个参数 props 对应父组件传入的 props 数据，第二个参数是一个对象，实际上就是 setupContext。
...
![](setup/Ciqc1F8VZvaAYCgKAAHVSzimXjw614.png)
